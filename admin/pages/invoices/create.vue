<template>
  <card-container title="Create Invoice">
    <template v-if="response !== null">
      <h5>{{ response.invoice.name }}</h5>
      <div>Amount: {{ $currency(response.invoice.amount_cents / 100) }}
        {{ response.invoice.currency }}
      </div>

      <div>{{ response.invoice.customer_name }} {{ response.invoice.customer_email }}</div>
      <div v-html="response.invoice.notes_html" v-if="response.invoice.notes_html"/>
      <div v-html="response.invoice.contents_html" v-if="response.invoice.contents_html"/>
