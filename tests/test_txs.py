...
from modules.electrum_mods.functions import construct_script, opcodes, script_to_p2wsh, \
    construct_witness, ecc
from modules.electrum_mods.tux_mods import network_context, deserialize_privkey
from modules.electrum_mods.tux_tx import PartialTransaction, Transaction, BCDataStream, SerializationError, \
    tx_from_any, convert_raw_tx_to_hex, get_address_from_output_script, TxOutpoint, PartialTxInput, PartialTxOutput
from tests import TestCaseForTestnet

BITCOIN = ALL_COINS['BTC']
BITCOIN_TESTNET = ALL_COINS['tBTC']
...
class TestTransaction(unittest.TestCase):
    ...
    def test_tx_update_signatures(self):
        ...
        tx.inputs()[0].pubkeys = [bytes.fromhex('02e61d176da16edd1d258a200ad9759ef63adf8e14cd97f53227bae35cdb84d2f6')]
        tx.inputs()[0].num_sig = 1
        tx.update_signatures(signed_blob_signatures, BITCOIN)
        self.assertEqual(tx.serialize(), signed_blob)

    @network_context(BITCOIN)
    ...
    def test_tx_deserialize_for_signed_network_tx(self):
        ...
        self.assertEqual('3140eb24b43386f35ba69e3875eb6c93130ac66201d01c58f598defc949a5c2a:0',
                         tx.inputs()[0].prevout.to_str())
        self.assertEqual(1, len(tx.outputs()))
        self.assertEqual(bytes.fromhex('76a914230ac37834073a42146f11ef8414ae929feaafc388ac'),
    ...
    def test_estimated_tx_size(self):
        ...
        self.assertEqual(tx.estimated_size(BITCOIN), 193)

    def test_estimated_output_size(self):
        estimated_output_size = Transaction.estimated_output_size
        self.assertEqual(estimated_output_size('14gcRovpkCoGkCNBivQBvw7eso7eiNAbxG', BITCOIN), 34)
        self.assertEqual(estimated_output_size('35ZqQJcBQMZ1rsv8aSuJ2wkC7ohUCQMJbT', BITCOIN), 32)
        self.assertEqual(estimated_output_size('bc1q3g5tmkmlvxryhh843v4dz026avatc0zzr6h3af', BITCOIN), 31)
        self.assertEqual(estimated_output_size('bc1qnvks7gfdu72de8qv6q6rhkkzu70fqz4wpjzuxjf6aydsx7wxfwcqnlxuv3',
                                               BITCOIN), 43)
    ...
    def test_tx_signed_segwit(self):
        ...
        self.assertEqual(tx.estimated_size(BITCOIN), 141)
    ...
    def test_get_address_from_output_script(self):
        ...
        addr_from_script = lambda script: get_address_from_output_script(bytes.fromhex(script), net=BITCOIN)

        # bech32/bech32m native segwit
        # test vectors from BIP-0173/BIP-0350
        self.assertEqual('bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4',
                         addr_from_script('0014751e76e8199196d454941c45d1b3a323f1433bd6'))
        self.assertEqual('bc1pw508d6qejxtdg4y5r3zarvary0c5xw7kw508d6qejxtdg4y5r3zarvary0c5xw7kt5nd6y', addr_from_script(
            '5128751e76e8199196d454941c45d1b3a323f1433bd6751e76e8199196d454941c45d1b3a323f1433bd6'))
        self.assertEqual('bc1sw50qgdz25j', addr_from_script('6002751e'))
        self.assertEqual('bc1zw508d6qejxtdg4y5r3zarvaryvaxxpcs',
                         addr_from_script('5210751e76e8199196d454941c45d1b3a323'))
        self.assertEqual('bc1p0xlxvlhemja6c4dqv22uapctqupfhlxm9h8z3k2e72q4k9hcz7vqzk5jj0',
                         addr_from_script('512079be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798'))
        ...
        # base58 p2pkh
        self.assertEqual('14gcRovpkCoGkCNBivQBvw7eso7eiNAbxG',
                         addr_from_script('76a91428662c67561b95c79d2257d2a93d9d151c977e9188ac'))
        self.assertEqual('1BEqfzh4Y3zzLosfGhw1AsqbEKVW6e1qHv',
        ...
        # base58 p2sh
        self.assertEqual('35ZqQJcBQMZ1rsv8aSuJ2wkC7ohUCQMJbT',
                         addr_from_script('a9142a84cf00d47f699ee7bbc1dea5ec1bdecb4ac15487'))
        self.assertEqual('3PyjzJ3im7f7bcV724GR57edKDqoZvH7Ji',
        ...
        self.assertEqual(None,
                         addr_from_script('210289e14468d94537493c62e2168318b568912dec0fb95609afd56f2527c2751c8bac'))
    ...
    def test_tx_from_any(self):
        ...
        raw_tx_map = {
            ...
            "network_tx_raw_bytes": RawTx(bytes.fromhex(
                ""),
            ...
            "psbt_raw_bytes": RawTx(bytes.fromhex(
                ""),
                is_whitespace_allowed=False),
        }
        whitespace_str = " \r\n  \n  "
    ...
    def _run_naive_tests_on_tx(self, raw_tx, txid):
        ...
        self.assertTrue(tx.estimated_size(BITCOIN) >= 0)
...
class TestTransactionTestnet(TestCaseForTestnet):
    ...
    def test_spending_op_cltv_p2sh(self):
        ...
        prevout = TxOutpoint(txid=bytes.fromhex('6d500966f9e494b38a04545f0cea35fc7b3944e341a64b804fed71cdee11d434'),
                             out_idx=1)
        txin = PartialTxInput(prevout=prevout)
        txin.nsequence = 2 ** 32 - 3
        txin.script_type = 'p2sh'
        redeem_script = bytes.fromhex(construct_script([
        ...
        sig = tx.sign_txin(0, privkey, BITCOIN)
        txin.script_sig = bytes.fromhex(construct_script([sig, redeem_script]))
    ...
    def test_spending_op_cltv_p2wsh(self):
        ...
        _, privkey, compressed = deserialize_privkey(wif)
        pubkey = ecc.ECPrivkey(privkey).get_public_key_hex(compressed=compressed)
        witness_script = bytes.fromhex(construct_script([
            locktime, opcodes.OP_CHECKLOCKTIMEVERIFY, opcodes.OP_DROP, pubkey, opcodes.OP_CHECKSIG,
        ]))
        from_addr = script_to_p2wsh(witness_script.hex())
        self.assertEqual("tb1q9dn6qke9924xe3zmptmhrdge0s043pjxpjndypgnu2t9fvsd4crs2qjuer", from_addr)
        prevout = TxOutpoint(txid=bytes.fromhex('8680971efd5203025cffe746f8598d0a704fae81f236ffe009c2609ec673d59a'),
                             out_idx=0)
        txin = PartialTxInput(prevout=prevout)
        ...
        sig = tx.sign_txin(0, privkey, BITCOIN)
        txin.witness = bytes.fromhex(construct_witness([sig, witness_script]))
